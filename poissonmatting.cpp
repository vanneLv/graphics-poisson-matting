...
void PoissonMatting::openFore(QString filename)
{
    img = cv::imread(std::string(filename.toLocal8Bit()));
}

void PoissonMatting::openBack(QString filename)
{
    background = cv::imread(std::string(filename.toLocal8Bit()));
}
...
cv::Mat &PoissonMatting::getImageCutMat()
{
    return imgCut;
}
...
cv::Mat &PoissonMatting::getbackMat()
{
    return background;
}

void PoissonMatting::checkImgTrimap(cv::InputArray _trimap)
{
    cv::Mat image = img;
...
//重载函数matting，用于localMatting
void PoissonMatting::matting(cv::InputArray _trimap, cv::OutputArray _alpha)
{
    cv::Mat image = imgCut;
    cv::Mat trimap = _trimap.getMat();
...
void PoissonMatting::globalMatting(cv::Mat _image, cv::Mat _trimap, cv::Mat &_foreground, cv::Mat &_alpha)
{
    const cv::Mat_<uchar> &image = static_cast<const cv::Mat_<uchar> &>(_image);
    const cv::Mat_<cv::Vec3b> &initImg = static_cast<cv::Mat_<cv::Vec3b>&>(img);
        ...
        for (int y = 0; y < trimap.rows; ++y) {
            for (int x = 0; x < trimap.cols; ++x) {
                ...
                if (trimap_blur(y, x) == 255) {
                    FminusB(y, x) = image(y, x) - 0.0;
                } else if (trimap_blur(y, x) == 0) {
                    FminusB(y, x) = 0.0 - image(y, x);
                } else {
                    // is in Unknown Area
                    ...
                    // Calculate F - B
                    FminusB(y, x) = (double)(image(nearestForegroundPoint.y, nearestForegroundPoint.x)-
        ...
        for (int times2 = 0; times2 < 300; ++times2) {
            for (int y = 1; y < trimap.rows - 1; ++y) {
                for (int x = 1; x < trimap.cols - 1; ++x) {
                    if (trimap(y, x) == 128) {
                        // is in Unknown Area
#define I(x, y) (image(inY(image, y), inX(image, x)))
...
#undef I
    ...
    //设置背景图,需要解决定位的问题
    //cv::Mat _back = cv::imread("C:\\Users\\Lv Mufan\\Desktop\\5.jpg");
    //cv::Mat_<cv::Vec3b> &back = static_cast<cv::Mat_<cv::Vec3b>&>(_back);
    // Generate Foreground Image (Red Background)
    for (int x = 0; x < alpha.cols; ++x) {
        for (int y = 0; y < alpha.rows; ++y) {
            foreground(y, x) = ((double) alpha(y, x) / 255) * initImg(y, x) + ((255.0 - alpha(y, x)) / 255 * cv::Vec3b(0, 0, 255));
            //foreground(y, x) = ((double) alpha(y, x) / 255) * image(y, x) + ((255.0 - alpha(y, x)) / 255 * back(y,x));
        }
    }
}

void PoissonMatting::localMatting(cv::Mat _image, cv::Mat _trimap, cv::Mat &_alpha)
{
    const cv::Mat_<uchar> &image = static_cast<const cv::Mat_<uchar> &>(_image);
    const cv::Mat_<cv::Vec3b> &initImg = static_cast<cv::Mat_<cv::Vec3b>&>(img);
    cv::Mat_<uchar> &trimap = static_cast<cv::Mat_<uchar> &>(_trimap);
